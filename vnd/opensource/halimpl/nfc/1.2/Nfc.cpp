...
Return<V1_0::NfcStatus> Nfc::open_1_1(
    ...
    if (clientCallback == nullptr) {
        ALOGD_IF(*getNfcDebugEnabled(), "Nfc::open null callback");
...
Return<V1_0::NfcStatus> Nfc::open(
    ...
    ALOGD_IF(*getNfcDebugEnabled(), "Nfc::open Enter");
    if (clientCallback == nullptr) {
        ALOGD_IF(*getNfcDebugEnabled(), "Nfc::open null callback");
        return V1_0::NfcStatus::FAILED;
    } else {
    ...
    NFCSTATUS status = tmsNciHalOpen(eventCallback, dataCallback);
    ALOGD_IF(*getNfcDebugEnabled(), "Nfc::open Exit");
    return CHK_STATUS(status);
}

Return<uint32_t> Nfc::write(const hidl_vec<uint8_t>& data) {
    hidl_vec<uint8_t> copy = data;
    return tmsNciHalWrite(copy.size(), &copy[0]);
}

Return<V1_0::NfcStatus> Nfc::coreInitialized(const hidl_vec<uint8_t>& data) {
    hidl_vec<uint8_t> copy = data;
    NFCSTATUS status = tmsNciHalCoreInitialized(copy.size(), &copy[0]);
    return CHK_STATUS(status);
}

Return<V1_0::NfcStatus> Nfc::prediscover() {
    NFCSTATUS status = tmsNciHalPreDiscover();
    return CHK_STATUS(status);
}
...
Return<V1_0::NfcStatus> Nfc::close() {
    ...
    NFCSTATUS status = tmsNciHalClose(false);
...
Return<V1_0::NfcStatus> Nfc::controlGranted() {
    NFCSTATUS status = tmsNciHalControlGranted();
    return CHK_STATUS(status);
}

Return<V1_0::NfcStatus> Nfc::powerCycle() {
    NFCSTATUS status = tmsNciHalPowerCycle();
    return CHK_STATUS(status);
}
...
Return<V1_0::NfcStatus> Nfc::closeForPowerOffCase() {
    ...
    NFCSTATUS status = tmsNciHalConfigDiscShutdown();
...
Return<void> Nfc::getConfig(getConfig_cb hidl_cb) {
    ...
    NfcCompat nfcCompat;
    nfcCompat.tmsNciHalGetVendorConfig(nfcVendorConfig);
...
Return<void> Nfc::getConfig_1_2(getConfig_1_2_cb hidl_cb) {
    ...
    NfcCompat nfcCompat;
    nfcCompat.tmsNciHalGetVendorConfig_1_2(nfcVendorConfig);
    hidl_cb(nfcVendorConfig);
    return Void();
}
