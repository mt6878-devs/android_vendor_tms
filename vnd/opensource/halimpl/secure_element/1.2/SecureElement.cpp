...
static const char TAG[] = "SecureElement1.2";
...
Return<void> SecureElement::transmit(const hidl_vec<uint8_t>& data,
  ...
  if (cmdApdu.len >= MIN_APDU_LENGTH) {
    ...
    if (cmdApdu.pData == NULL) {
      TMS_LOG_E(TAG, "%s: malloc cmdApdu.pData failed", __func__);
      _hidl_cb(result);
      return Void();
    }
...
SecureElement::closeChannel(uint8_t channelNumber) {
  ...
  if ((channelNumber >= MAX_LOGICAL_CHANNELS) ||
    ...
    if (status != ESESTATUS_SUCCESS) {
      ...
      // status is ESESTATUS_SUCCESS, sestatus should be assigned success state.
      sestatus = SecureElementStatus::SUCCESS;
    }
    free(cmdApdu.pData);
    free(rspApdu.pData);
