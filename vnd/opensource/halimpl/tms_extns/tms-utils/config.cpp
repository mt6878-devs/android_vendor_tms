...
bool parseBytesString(std::string in, std::vector<uint8_t>& out)
{
    ...
    for (string value : values) {
        ...
        if (!ParseUint(hexified.c_str(), &tmp)) {
            return false;
        }
        out.push_back(tmp);
    }
...
bool ConfigValue::parseFromString(std::string in)
{
    if (in.length() > 1 && in[0] == '"' && in[in.length() - 1] == '"') {
        CHECK(in.length() > 2);  // length() > 2
        type_ = STRING;
        value_string_ = in.substr(1, in.length() - 2);  // length() > 2
        return true;
    }

    if (in.length() > 1 && in[0] == '{' && in[in.length() - 1] == '}') {
        CHECK(in.length() >= 4);  // Needs at least one byte, length() > 4
        type_ = BYTES;
        // length() - 2
