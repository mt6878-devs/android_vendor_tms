...
extern SyncEvent sNfaTransitConfigEvent;
extern void nfcManager_abortWaits();
#endif
...
static void nfaConnectionCallback(uint8_t connEvent,
  ...
  switch (connEvent) {
    ...
    case NFA_ACTIVATED_EVT:  // NFC link/protocol activated
    {
      ...
      if (gIsSelectingRfInterface) {
#ifdef TMS_NFC
        if (needUpdateTagData()) {
          NfcTag::getInstance().connectionEventHandler(NFA_ACTIVATED_UPDATE_EVT, eventData);
          if (NfcTag::getInstance().getNumDiscNtf()) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
              "%s: NFA_ACTIVATED_EVT: A multi-protocol card is found in the reselect process, deactivate it and select the next protocol",__func__);
...
static jboolean nfcManager_routeAid(JNIEnv* e, jobject, jbyteArray aid,
  ...
  bufLen = bytes.size();
#ifdef TMS_NFC
  if (route == T4TNFCEE_TARGET_HANDLE) {
...
static jboolean nfcManager_doInitialize(JNIEnv* e, jobject o) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: NCI MW Version: %s", __func__, mw_version);
  DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: NCI MW Build Time: %s", __func__, mw_build_time);
#endif
...
#ifdef TMS_NFC
        tms::SecureElement::getInstance().initialize();
...
#ifdef TMS_NFC
  tms::SecureElement::getInstance().deinitialize();
  NativeT4tNfcee::getInstance().onNfccShutdown();
#endif
...
#ifdef TMS_NFC
  tms::SecureElement::getInstance().abortWaits();
  nfcManager_abortWaits();
  {
    // unlock NFA_SetConfig()
    SyncEventGuard guard(gNfaSetConfigEvent);
    gNfaSetConfigEvent.notifyOne();
  }
#endif
