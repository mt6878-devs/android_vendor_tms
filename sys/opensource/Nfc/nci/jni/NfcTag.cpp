  ...
  if (NFC_PROTOCOL_T1T == rfDetail.protocol) {
    ...
    mTechList[mNumTechList] =
        TARGET_TYPE_MIFARE_CLASSIC;  // is TagTechnology.MIFARE_CLASSIC by Java
                                     // API
#ifdef TMS_NFC
  } else if ((NCI_PROTOCOL_UNKNOWN == rfDetail.protocol) &&
...
void NfcTag::fillNativeNfcTagMembers4(JNIEnv* e, jclass tag_cls, jobject tag,
  ...
  if (mTechListTail == 0) {
    ...
    for (int j = 0; j < mTechListTail; j++) {
      ...
      e->SetObjectArrayElement(techActBytes.get(), j, gtechActBytesObject);
#else
      if (NFC_PROTOCOL_T1T == mTechLibNfcTypes[j] ||
         NFC_PROTOCOL_T2T == mTechLibNfcTypes[j] ||
         (NFC_PROTOCOL_ISO_DEP == mTechLibNfcTypes[j] && TARGET_TYPE_ISO14443_3A == mTechList[j])) {
        DLOG_IF(INFO, true) << StringPrintf("%s: keep NFC_A sak", fn);
      } else {
        gtechActBytesObject = e->GetObjectArrayElement(gtechActBytes, j);
        e->SetObjectArrayElement(techActBytes.get(), j, gtechActBytesObject);
      }
...
/*******************************************************************************
**
** Function:        setTagSkipProtocol
**
** Description:     This interface is used to set cards for the specified protocol
**                  to be ignored when multi-protocol cards are detected
**
** Returns:         None
**
*******************************************************************************/
void NfcTag::setTagSkipProtocol(int protocol) {
  static const char fn[] = "NfcTag::setTagSkipProtocol";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: protocol=0x%02x", fn, protocol);
  mTagSkipProtocol = protocol;
}
...
void NfcTag::selectTag(int idx) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: idx=%d, mNumDiscNtf=%d", fn, idx, mNumDiscNtf);

  int foundIdx = -1;
  tNFA_INTF_TYPE rf_intf = NFA_INTERFACE_FRAME;

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, mNumDiscTechList=%x, idx=%d, mTagSkipProtocol=%d",
          fn, mNumDiscTechList, idx, mTagSkipProtocol);

  if (idx < mNumDiscTechList) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: nfa target idx=%d h=0x%X; protocol=0x%X", fn, idx,
                        mTechHandlesDiscData[idx], mTechLibNfcTypesDiscData[idx]);
    if (mTechLibNfcTypesDiscData[idx] != NFA_PROTOCOL_NFC_DEP) {
      if (mTagSkipProtocol != -1) {
        if (mTechLibNfcTypesDiscData[idx] != mTagSkipProtocol) {
  ...
  if (foundIdx != -1) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "selectTag: mTechHandles[foundIdx]=0x%X, "
        "mTechLibNfcTypes[foundIdx]=0x%x, "
        "rf_intf=0x%x, foundIdx=0x%x",
        mTechHandles[foundIdx], mTechLibNfcTypes[foundIdx], rf_intf, foundIdx);
...
void NfcTag::selectFirstTag() {
  ...
  if (mTagSkipProtocol != -1) {
    for (int i = 0; i < mNumDiscTechList; i++) {
      if (mTechLibNfcTypesDiscData[i] == mTagSkipProtocol) {
        skipIndex = i;
        break;
      }
    }
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: mNumTechList=%d, mTagSkipProtocol=%d, mNumDiscNtf=%d", fn, mNumTechList, mTagSkipProtocol, mNumDiscNtf);
#endif
  ...
  for (int i = 0; i < mNumDiscTechList; i++) {
    ...
    if (mTechLibNfcTypesDiscData[i] != NFA_PROTOCOL_NFC_DEP) {
      ...
      if (skipIndex != i) {
          ...
          DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: skip : 0x%X", fn, mTechLibNfcTypesDiscData[i]);
      }
#else
      sLastSelectedTagId = i;
      foundIdx = i;
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: skipIndex=%d, mNumDiscNtf=%d, NFC_PROTOCOL_MIFARE=0x%x, mNumDiscTechList=%d", fn, skipIndex, mNumDiscNtf, NFC_PROTOCOL_MIFARE, mNumDiscTechList);
  if (skipIndex!=-1 && mNumDiscNtf > 0) {
    mNumDiscNtf--;
  }
  //mTagSkipProtocol = -1;
#endif
...
void NfcTag::connectionEventHandler(uint8_t event, tNFA_CONN_EVT_DATA* data) {
  ...
  switch (event) {
    ...
    case NFA_ACTIVATED_UPDATE_EVT: {
      LOG(INFO) << StringPrintf("%s: NFA_ACTIVATED_UPDATE_EVT called", fn);
      tNFA_ACTIVATED& activated = data->activated;
      mIsActivated = true;
      mProtocol = activated.activate_ntf.protocol;
      mTechListTail = mNumTechList;
      LOG(INFO) << StringPrintf("%s: NFA_ACTIVATED_UPDATE_EVT called, mTechListTail = %d", fn, mTechListTail);
      calculateT1tMaxMessageSize(activated);
      discoverTechnologies(activated);
      if (mNumDiscNtf == 0) mTechListTail = 0;
    } break;
...
#ifdef TMS_NFC
bool NfcTag::isMultiProtocolSupported() {
  static const char fn[] = "NfcTag::isMultiProtocolSupported";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: return=%d", fn, mIsMultiProtocolTag);
  return mIsMultiProtocolTag;
}
