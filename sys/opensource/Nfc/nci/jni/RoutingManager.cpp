...
//TMS_NFC for route setting
#ifdef TMS_NFC
Mutex gMutexEE;
static Mutex sEeInfoMutex;
namespace tms {
extern void onNfceeActionEvent(tNFA_EE_ACTION& action);
}
bool mIsNfccShutdown = false;
#endif
...
const JNINativeMethod RoutingManager::sMethods[] = {
    ...
    {"doGetDefaultSysCodeRouteDestination", "()I",
     (void*)RoutingManager::
         com_android_nfc_cardemulation_doGetDefaultSysCodeRouteDestination}
#endif
         };
//TMS_NFC for route setting
#ifndef TMS_NFC
...
//TMS_NFC for route setting
  ...
  mHceTechMask = NfcConfig::getUnsigned(NAME_TMS_DEFAULT_HCE_TECH, NFA_TECHNOLOGY_MASK_A|NFA_TECHNOLOGY_MASK_B);
#endif
...
//TMS_NFC for route setting
#ifdef TMS_NFC
  memset(&mEeInfo1, 0, sizeof(mEeInfo1));
#endif
...
#ifdef TMS_NFC
void RoutingManager::setEmptyAidEntry(int route, int power) {
  static const char fn[] = "RoutingManager::setEmptyAidEntry";
  DLOG_IF(INFO, nfc_debug_enabled) << fn << ": enter";
  addAidRouting(NULL, 0, route, AID_ROUTE_QUAL_PREFIX, mSecureNfcEnabled ? 0x01 : power);
}
#endif
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: mEeInfoChanged=%d, mAidRoutingConfigured=%d", fn, mEeInfoChanged, mAidRoutingConfigured);
...
//TMS_NFC for route setting
  ...
  int connectedIsoDepRoute =
      RoutingManager::getInstance().getConnectedNfceeId(isoDepRoute);
...
**
*******************************************************************************/
int RoutingManager::getScTypeFRouting(eJNI_ROUTING_TYPE type) {
  static const char fn[] = "RoutingManager::getScTypeFRouting";
  ...
  if (type == ROUTING_TYPE_F) {
    ...
    if (mUserDefaultFelicaRoute != INVALID_ROUTE_VALUE) {
      //      DLOG_IF(INFO, nfc_debug_enabled)
      //          << StringPrintf("%s: User modified Felica route = 0x%02X",
      //                          fn, mUserDefaultFelicaRoute);
    ...
    if (mUserDefaultScRoute != INVALID_ROUTE_VALUE) {
      //      DLOG_IF(INFO, nfc_debug_enabled)
      //          << StringPrintf("%s: User modified System Code route =
      //          0x%02X",
      //                          fn, mUserDefaultScRoute);
      route = mUserDefaultScRoute;
    }
  }

  connectedRoute = RoutingManager::getInstance().getConnectedNfceeId(route);

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s: type = %s, requested route = 0x%02X, "
      "connected route = 0x%02X",
      fn,
      ...
      for (uint8_t i = 0; i < localEeInfo.num_ee; i++) {
        ...
        if (eeHandle == (connectedRoute | NFA_HANDLE_GROUP_EE)) {
          ...
          if (localEeInfo.ee_disc_info[i].lf_protocol != 0) {
            ...
            // If UICC
            if (((connectedRoute == 0xC0)
              //&&(mIsSEFelicaCard)
              ) || (connectedRoute == 0x80) || (connectedRoute == 0x81)) {
              DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
...
//TMS_NFC for route setting
  ...
  //     RoutingManager::getInstance().getConnectedNfceeId(aidRoute);
  ...
  // No need to set empty aid here because empty aid will be set in java layer every time
  // DLOG_IF(INFO, nfc_debug_enabled)
...
//TMS_NFC for route setting
  ...
  int connectedDefaultOffHostRoute =
      RoutingManager::getInstance().getConnectedNfceeId(offHostRoute);
...
**
*******************************************************************************/
void RoutingManager::updateFullRoutes() {
  static const char fn[] = "RoutingManager::updateFullRoutes()";
  tNFA_STATUS nfaStat;
...
DLOG_IF(INFO, nfc_debug_enabled)
        << fn
        << StringPrintf(" connectedDefaultEe = 0x%02X mUserDefaultAidRoute = 0x%02X aidRoute= 0x%02X",connectedDefaultEe,
  ...
  if (felicaRoute != -1) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << fn << StringPrintf(" - No Felica route");
  }
  if (sysCodeRoute != -1) {
    DLOG_IF(INFO, nfc_debug_enabled)
  ...
  if (mUserDefaultMifareRoute != INVALID_ROUTE_VALUE) {
    ...
    connectedMifareRoute = RoutingManager::getInstance().getConnectedNfceeId(
        mUserDefaultMifareRoute);
  }
  ...
  uint8_t powerState = 0x01;
  if (!mSecureNfcEnabled)
  powerState = (connectedDefaultEe != 0x00) ? mOffHostAidRoutingPowerState : 0x11;
      nfaStat = NFA_EeAddAidRouting(connectedDefaultEe, 0, NULL, powerState,
                AID_ROUTE_QUAL_PREFIX);
  if (nfaStat == NFA_STATUS_OK)
      DLOG_IF(INFO, nfc_debug_enabled)
  ...
  /**************************************************************/
  {
    ...
    // Route Nfc-A to host if we don't have a SE
    tNFA_TECHNOLOGY_MASK techMask = NFA_TECHNOLOGY_MASK_A;
    if ((mHceTechMask & NFA_TECHNOLOGY_MASK_A) &&
        ((mSeTechMask & NFA_TECHNOLOGY_MASK_A) == 0)) {
      nfaStat = NFA_EeSetDefaultTechRouting(
          NFC_DH_ID, techMask, 0, 0, mSecureNfcEnabled ? 0 : techMask, 0, 0);
      if (nfaStat == NFA_STATUS_OK)
    ...
    techMask = NFA_TECHNOLOGY_MASK_B;
    if ((mHceTechMask & NFA_TECHNOLOGY_MASK_B) &&
        ((mSeTechMask & NFA_TECHNOLOGY_MASK_B) == 0)) {
      nfaStat = NFA_EeSetDefaultTechRouting(
          NFC_DH_ID, techMask, 0, 0, mSecureNfcEnabled ? 0 : techMask, 0, 0);
      if (nfaStat == NFA_STATUS_OK)
 ...
 **
 *******************************************************************************/
int RoutingManager::retrieveHostList(uint8_t* ptrHostList, uint8_t* ptrInfo) {
  static const char fn[] = "SecureElement::retrieveHostList()";
  ...
  tNFA_STATUS nfaStat = NFA_STATUS_FAILED;
  gMutexEE.lock();
  if ((nfaStat = NFA_EeGetInfo(&lActualNumEe, mEeInfo1)) != NFA_STATUS_OK) {
    LOG(ERROR) << StringPrintf("%s: fail get info; error=0x%X", fn, nfaStat);
    gMutexEE.unlock();
    return 0;
  }
  ...
  for (i = 0; i < lActualNumEe; i++) {
    if (mEeInfo1[i].ee_handle & 0x80) {
      ptrHostList[idx] = mEeInfo1[i].ee_handle & ~NFA_HANDLE_GROUP_EE;
      ptrInfo[idx] = mEeInfo1[i].ee_status;
      idx++;
    }
  }
  return idx;
...
uint8_t RoutingManager::getConnectedNfceeId(uint8_t id) {
  static const char fn[] = "SecureElement::getConnectedNfceeId";
...
#ifdef TMS_NFC
  tms::SecureElement::getInstance().nfaEeCallback(event, eventData);
#endif
}
...
**
*******************************************************************************/
void RoutingManager::setUserDefaultRoutesPref(int mifareRoute,
                                                int isoDepRoute,
                                                int felicaRoute,
                                                int abTechRoute, int scRoute,
                                                int aidRoute) {
  static const char fn[] = "RoutingManager::setUserDefaultRoutesPref";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
...
int RoutingManager::
    com_android_nfc_cardemulation_doGetDefaultSysCodeRouteDestination(JNIEnv*) {
