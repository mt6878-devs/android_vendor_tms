...
#ifdef TMS_NFC
  void setEmptyAidEntry(int route, int power);
#endif
...
//TMS_NFC for route setting
  ...
  static int com_android_nfc_cardemulation_doGetDefaultFelicaRouteDestination(
      JNIEnv* e);
  static int com_android_nfc_cardemulation_doGetDefaultSysCodeRouteDestination(
      JNIEnv* e);
#endif
...
//TMS_NFC for route setting
  ...
  tNFA_EE_INFO mEeInfo1[MAX_NUM_EE];  // actual size stored in mActualNumEe

  bool mScRoutingConfigured;
  tNFA_TECHNOLOGY_MASK mHceTechMask;
#endif
