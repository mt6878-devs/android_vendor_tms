...
#ifdef TMS_NFC
static bool sTmsInRetryReselect = false;
#endif
}  // namespace android
...
static int reSelect(tNFA_INTF_TYPE rfInterface, bool fSwitchIfNeeded) {
  ...
  int cachedConnectedTargetType = natTag.mTechList[sCurrentConnectedHandle];
  int cachedConnectedTargetHandles = natTag.mTechHandles[sCurrentConnectedHandle];
  int cachedConnectedTargetProtocol = natTag.mTechLibNfcTypes[sCurrentConnectedHandle];
  int cachedRfInterface = sCurrentRfInterface;
#endif

  do {
#ifdef TMS_NFC
    DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: ndef detection timeout; ignore", __func__);
#else
...
#ifdef TMS_NFC
  if (rVal == 0 && sCurrentConnectedTargetProtocol == NFC_PROTOCOL_UNKNOWN) {
      DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: recovery state, sCurrentConnectedHandle = %d, cachedConnectedTargetType = %d, cachedConnectedTargetProtocol = %d, cachedConnectedTargetHandles = %d", __func__,
          sCurrentConnectedHandle, cachedConnectedTargetType, cachedConnectedTargetProtocol, cachedConnectedTargetHandles);
      DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: current techlist: 0x%02X, 0x%02X, 0x%02X", __func__, 
          natTag.mTechList[sCurrentConnectedHandle], natTag.mTechHandles[sCurrentConnectedHandle], natTag.mTechLibNfcTypes[sCurrentConnectedHandle]);

      if (natTag.isMultiProtocolSupported()) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: multi-protocol card, sCurrentActivatedProtocl = %d" ,__func__, sCurrentActivatedProtocl);
        sCurrentConnectedTargetType = cachedConnectedTargetType;
        sCurrentConnectedTargetProtocol = cachedConnectedTargetProtocol;
        if (sCurrentConnectedTargetProtocol != sCurrentActivatedProtocl) {
          needRetry = true;
          DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: multi-protocol card, need retry" ,__func__);
        } else {
            DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: multi-protocol card, no need retry" ,__func__);
        }
      } else {
        if ((cachedConnectedTargetType != natTag.mTechList[sCurrentConnectedHandle]) || (cachedConnectedTargetProtocol != natTag.mTechLibNfcTypes[sCurrentConnectedHandle])) {
          rVal = NFA_STATUS_FAILED;
          DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: card information has changed, aborting the recovery" ,__func__);
        } else {
          sCurrentConnectedTargetType = natTag.mTechList[sCurrentConnectedHandle];
          sCurrentConnectedTargetProtocol = natTag.mTechLibNfcTypes[sCurrentConnectedHandle];
          DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: recovery state done" ,__func__);
        }
      }
  }
#endif
  ...
  sRfInterfaceMutex.unlock();
#ifdef TMS_NFC
  if (needRetry && !sTmsInRetryReselect) {
    sTmsInRetryReselect = true;
    sCurrentRfInterface = cachedRfInterface;
    rVal = reSelect(rfInterface, fSwitchIfNeeded);
  }
  sTmsInRetryReselect = false;
#endif
  DLOG_IF(INFO, nfc_debug_enabled)
...
void nativeNfcTag_doSelectTag() {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: sCurrentConnectedHandle = %d", __func__, sCurrentConnectedHandle);
  NfcTag::getInstance().selectTag(sCurrentConnectedHandle);
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: exit", __func__);
}

bool needUpdateTagData() {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: enter, sCurrentConnectedTargetProtocol = %d", __func__, sCurrentConnectedTargetProtocol);

  return sCurrentConnectedTargetProtocol == NFC_PROTOCOL_UNKNOWN;
}
#endif
