...
import com.tms.nfc.TmsConfig;
import com.tms.nfc.TmsNfcAdapter;
import com.tms.nfc.TmsNfcService;
// #endif
...
public class PreferredServices implements com.android.nfc.ForegroundUtils.Callback {
    ...
    static final boolean DBG = com.android.nfc.NfcService.DBG; // TMS_NFC
    ...
    // #ifdef TMS_NFC
    private static final HashMap<String, String> sPaymentRouteTable = new HashMap<>();
    private static final int SET_ALL_ROUTE = TmsConfig.getInt("TMS_SET_ALL_ROUTE_DEFAULT", 1);

    static {
        sPaymentRouteTable.put("com.cmcc.hebao/com.cmcc.wallet.mocam.service.WalletOffHostApduService", TmsNfcAdapter.UICC_ROUTE);
        sPaymentRouteTable.put("com.comprion.android.myOffHostService/com.comprion.android.util.MyOffHostApduService", TmsNfcAdapter.UICC_ROUTE);
    }
    // #endif
    ...
    void loadDefaultsFromSettings(int userId, boolean force) {
        // #ifdef TMS_NFC
        Log.d(TAG, "loadDefaultsFromSettings() called with: userId = [" + userId + "]");
        // #endif
        ...
        // #ifdef TMS_NFC
        computeDefaultRouteFromDefaultPayment();
        // #endif
    ...
    // #ifdef TMS_NFC
    public void computeDefaultRouteFromDefaultPayment() {
        if (SET_ALL_ROUTE == 0) {
            if (DBG) {
                Log.d(TAG, "do not set all route according to default payment.");
            }
            return;
        }
        ...
        String route = null;
        if (newDefault == null) {
            Log.e(TAG, "The default payment app is [" + name + "], change route to default from config");
            route = TmsNfcAdapter.DEFAULT_ROUTE;
        } else {
            ApduServiceInfo service = mServiceCache.getService(ActivityManager.getCurrentUser(), newDefault);
            Log.i(TAG, "computeDefaultRouteFromDefaultPayment payment name=" + name);
            Log.i(TAG, "computeDefaultRouteFromDefaultPayment apdu service =" + service);
            if (sPaymentRouteTable.containsKey(name)) {
                route = sPaymentRouteTable.get(name);
                Log.i(TAG, "computeDefaultRouteFromDefaultPayment match whitelist, route = " + route);
            }
            if (service != null && route == null) {
                String offHostSEName = service.getOffHostSecureElement();
                if (service.isOnHost() || (offHostSEName == null)) {
                    //HCE
                    route = TmsNfcAdapter.HCE_ROUTE;
                } else if (offHostSEName.startsWith("eSE")) {
                    //eSE
                    route = TmsNfcAdapter.ESE_ROUTE;
                } else if (offHostSEName.equals("SIM2")) {
                    //SIM2
                    route = TmsNfcAdapter.UICC2_ROUTE;
                } else {
                    //SIM1
                    route = TmsNfcAdapter.UICC_ROUTE;
                }
            }
        }
        if (route != null) {
            // TmsNfcService may be null when CardEmulationManager is not ready.
            if (NfcService.getInstance().getTmsNfcService() != null) {
                NfcService.getInstance().getTmsNfcService().setAllRouteToPrefs(route);
            } else {
                TmsNfcService.setInitDefaultRoute(route);
            }
        }
    }
    // #endif
