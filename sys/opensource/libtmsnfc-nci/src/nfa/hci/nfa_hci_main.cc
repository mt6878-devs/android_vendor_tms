...
#ifdef TMS_NFC
static void nfa_hci_nfcee_pl_cfg();
#endif
...
#ifdef TMS_NFC
      nfa_hciu_alloc_gate(NFA_HCI_APP_APDU_GATE, 0);
#endif
    }
...
/*******************************************************************************
**
** Function         nfa_hci_nfcee_pl_cfg
**
** Description      action function to set power link command for eSE
**
** Returns          None
**
*******************************************************************************/
void nfa_hci_nfcee_pl_cfg() {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s Enter.", __func__);
  tNFA_HANDLE nfcee_id = NFA_HCI_HOST_ID_PROP_HOST0|NFA_HANDLE_GROUP_EE;
  if (!nfa_ee_is_active(nfcee_id)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s eSE is not active.", __func__);
    return;
  }
  //check if need to set value from config
  if (NfcConfig::hasKey(NAME_TMS_NFCEE_PL_CFG)) {
    uint8_t nfcee_pl_cfg = NfcConfig::getUnsigned(NAME_TMS_NFCEE_PL_CFG);
    if (nfcee_pl_cfg == 0x01)
    {
      uint8_t nfcee_pl_val = 0xFF;  //init a invaild value
      if (NfcConfig::hasKey(NAME_TMS_NFCEE_PL_VALUE))
      {
        nfcee_pl_val = NfcConfig::getUnsigned(NAME_TMS_NFCEE_PL_VALUE);
      }
      switch (nfcee_pl_val)
      {
      case 0x00:
      ...
      case 0x03:
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "Set nfcee pl = %u", nfcee_pl_val);
        nfa_ee_cb.ese_prv_pwr_cfg = nfcee_pl_val;
        break;
      default:
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "Invaild nfcee pl = %u", nfcee_pl_val);
        break;
      }
    }
  }
...
void nfa_hci_rsp_timeout() {
  ...
  switch (nfa_hci_cb.hci_state) {
    ...
    case NFA_HCI_STATE_WAIT_RSP:
      ...
      break;
#ifdef TMS_NFC
    case NFA_HCI_STATE_WAIT_ATR:
      nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
      nfa_hci_handle_get_atr_timeout();
      break;

    case NFA_HCI_STATE_WAIT_RAPDU:
      nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
