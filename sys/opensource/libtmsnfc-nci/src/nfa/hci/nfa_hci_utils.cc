...
std::string nfa_hciu_get_event_name(uint16_t event) {
  switch (event) {
    ...
    case NFA_HCI_API_ABORT_APDU_EVT:
      return "ABORT_APDU_EVT";
    case NFA_HCI_API_SEND_APDU_EVT:
      return "SEND_APDU_EVT";
...
tNFA_STATUS nfa_hciu_send_raw_data(uint8_t *p_buf, uint8_t data_len, tNFA_VSC_CBACK * p_cback)
{
    ...
    status = NFA_SendRawVsCommand(data_len, p_buf, p_cback);
...
exit:
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfa_hciu_send_raw_data ret:%d len:%d", status, data_len);
    return status;
}
#endif
