...
static void nfa_hci_handle_generic_gate_evt(uint8_t* p_data, uint16_t data_len,
                                            ...
                                            tNFA_HCI_DYN_PIPE* p_pipe);
#ifdef TMS_NFC
static void nfc_hci_get_apdu_pipe_status_cb(uint8_t event, uint16_t param_len,
                                            uint8_t *p_param);
static void nfa_hci_handle_apdu_gate_pkt(uint8_t* p_data,
                                            uint16_t data_len,
                                            tNFA_HCI_DYN_PIPE* p_pipe);
...
void nfa_hci_check_pending_api_requests(void) {
  ...
  switch (p_msg->event) {
    ...
    case NFA_HCI_API_ABORT_APDU_EVT:
        if (nfa_hci_api_apdu_abort(p_evt_data) == false) b_free = false;
      break;
    case NFA_HCI_API_SEND_APDU_EVT:
        if (nfa_hci_api_send_apdu(p_evt_data) == false) b_free = false;
      break;
    case NFA_HCI_API_END_APDU_EVT:
        if (nfa_hci_api_end_apdu(p_evt_data) == false) b_free = false;
#endif
  }
...
#ifdef TMS_NFC
      case NFA_HCI_API_ABORT_APDU_EVT:
        nfa_hci_api_apdu_abort(p_evt_data);
        break;
      case NFA_HCI_API_SEND_APDU_EVT:
        nfa_hci_api_send_apdu(p_evt_data);
        break;
      case NFA_HCI_API_END_APDU_EVT:
        nfa_hci_api_end_apdu(p_evt_data);
        break;
#endif
...
#ifdef TMS_NFC
          // for wired mode
          nfa_hci_check_and_create_apdu_pipe();
#endif
...
#ifdef TMS_NFC
  } else if (p_pipe->local_gate == NFA_HCI_APP_APDU_GATE) {
    nfa_hci_handle_apdu_gate_pkt(p_data, data_len, p_pipe);
#endif
  } else {
    p_gate = nfa_hciu_find_gate_by_gid(p_pipe->local_gate);
    if (p_gate == nullptr) {
...
/*******************************************************************************
**
** Function         nfa_hci_get_apdu_pipe_status_from_nfcc
**
** Description      get apdu pipe status from NFCC by NCI
**
** Returns          return NFA_STATUS_OK on data sent success
**
*******************************************************************************/
tNFA_STATUS nfa_hci_get_apdu_pipe_status_from_nfcc()
{
    tNFA_STATUS status = NFA_STATUS_OK;
    uint8_t cmd_get_apdu_pipe_status[] = {0x20, 0x03, 0x03, 0x01, 0xA0, 0x23};

    status = nfa_hciu_send_raw_data(cmd_get_apdu_pipe_status, sizeof(cmd_get_apdu_pipe_status), nfc_hci_get_apdu_pipe_status_cb);
    return status;
}

/*******************************************************************************
**
** Function         nfc_hci_get_apdu_pipe_status_cb
**
** Description      callback to handle getting apdu pipe status rsp
**                  create apdu pipe if it is not ready
**
*******************************************************************************/
static void nfc_hci_get_apdu_pipe_status_cb(uint8_t event, uint16_t param_len,
                                            uint8_t *p_param)
{
    uint8_t apdu_pipe_status_rsp_pattern[] = {0x40, 0x03, 0x06, 0x00, 0x01, 0xA0, 0x23, 0x01, 0x00};
    uint8_t apdu_pipe_status = 0;
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("nfc_hci_get_apdu_pipe_status_cb: event:%d", event);

    nfa_sys_stop_timer(&nfa_hci_cb.timer);

    // RSP: 4003060001A02301XX
    if ((param_len < sizeof(apdu_pipe_status_rsp_pattern)) ||
        (0 != memcmp(apdu_pipe_status_rsp_pattern, p_param, sizeof(apdu_pipe_status_rsp_pattern) - 1))) {
        LOG(ERROR) << StringPrintf("nfc_hci_get_apdu_pipe_status_cb: get apsu pipe status failed");
        return;
    }

    // get apdu pipe status from rsp
    apdu_pipe_status = p_param[sizeof(apdu_pipe_status_rsp_pattern) - 1];
    if (0 != apdu_pipe_status) {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("apdu pipe is ready");
        // update apdu pipe status
        nfa_hciu_add_pipe_to_gate(NFA_HCI_APDU_PIPE_ID, NFA_HCI_APP_APDU_GATE, NFA_HCI_FIRST_PROP_HOST, NFA_HCI_APDU_GATE);
        return;
    }

    LOG(INFO) << StringPrintf("apud pipe is not created yet. create it");
    // check if eSE is activated
    if (!nfa_hciu_is_active_host(NFA_HCI_FIRST_PROP_HOST)) {
        LOG(ERROR) << StringPrintf("eSE is not activated. stop creating apdu pipe");
        return;
    }
    // create apdu pipe
    nfa_hciu_send_create_pipe_cmd(NFA_HCI_APP_APDU_GATE, NFA_HCI_FIRST_PROP_HOST, NFA_HCI_APDU_GATE);
}

/*******************************************************************************
**
** Function         nfa_hci_check_and_create_apdu_pipe
**
** Description      This function checks if apdu pipe is ready
**                  Create apdu pipe if it is not ready
**
*******************************************************************************/
void nfa_hci_check_and_create_apdu_pipe()
{
    LOG(ERROR) << StringPrintf("nfa_hci_check_and_create_apdu_pipe");

    nfa_hci_get_apdu_pipe_status_from_nfcc();
}

/*******************************************************************************
**
** Function         nfa_hci_api_apdu_abort
**
** Description      send abort apdu resuest
**
** Returns          return true on message sent success
**
*******************************************************************************/
bool nfa_hci_api_apdu_abort(tNFA_HCI_EVENT_DATA *p_evt_data)
{
    UNUSED(p_evt_data);
    tNFA_STATUS status;

    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_api_apdu_abort");
    if (nfa_hci_cb.hci_state == NFA_HCI_STATE_IDLE) {
        status = nfa_hciu_send_msg (NFA_HCI_APDU_PIPE_ID, NFA_HCI_EVENT_TYPE, NFA_HCI_EVT_ABORT, 0, 0);
        if (NFA_STATUS_OK != status) {
            LOG(ERROR) << StringPrintf("nfa_hci_api_apdu_abort: send message failed");
            nfa_hci_handle_get_atr_timeout();
            goto exit;
        }
        nfa_hci_cb.hci_state = NFA_HCI_STATE_WAIT_ATR;
        nfa_sys_start_timer(&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT,
            p_evt_data->abort_apdu.response_timeout);
    } else {
        LOG(WARNING) << StringPrintf("nfa_hci_api_apdu_abort: warning:hci state is not expected. state:%d => %s",
            nfa_hci_cb.hci_state, nfa_hciu_get_state_name(nfa_hci_cb.hci_state).c_str());
        nfa_hci_handle_get_atr_timeout();
    }
exit:
    return true;
...
/*******************************************************************************
**
** Function         nfa_hci_handle_apdu_gate_pkt
**
** Description      handle message from apdu gate
**                  and dispatch to upper layer
**
*******************************************************************************/
static void nfa_hci_handle_apdu_gate_pkt(uint8_t* p_data,
                                                 uint16_t data_len,
                                                 tNFA_HCI_DYN_PIPE* p_pipe) {
    tNFA_HCI_EVT_DATA evt_data;
    if (p_pipe->local_gate != NFA_HCI_APP_APDU_GATE) {
        LOG(ERROR) << StringPrintf("nfa_hci_handle_apdu_gate_pkt: not apdu gate data");
        return;
    }

    DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfa_hci_handle_apdu_gate_pkt: type:%02X hci_state:%d => %s",
            nfa_hci_cb.type, nfa_hci_cb.hci_state, nfa_hciu_get_state_name(nfa_hci_cb.hci_state).c_str());
    if (NFA_HCI_EVENT_TYPE == nfa_hci_cb.type) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfa_hci_handle_apdu_gate_pkt: handle event. inst:%02X", nfa_hci_cb.inst);
        switch (nfa_hci_cb.inst) {
            case NFA_HCI_EVT_ATR:
                nfa_sys_stop_timer(&nfa_hci_cb.timer);
                if (NFA_HCI_STATE_WAIT_ATR == nfa_hci_cb.hci_state) {
                    nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
                    evt_data.abort_apdu.status = NFA_STATUS_OK;
                    evt_data.abort_apdu.atr_len = data_len;
                    evt_data.abort_apdu.p_atr = p_data;
                    evt_data.abort_apdu.host_id = p_pipe->dest_host;
                    nfa_hciu_send_to_all_apps(NFA_HCI_ABORT_APDU_EVT, &evt_data);
                } else {
                    LOG(ERROR) << StringPrintf("hci stack is not waiting for ATR, drop it");
                }
                break;

            case NFA_HCI_EVT_R_APDU:
                nfa_sys_stop_timer(&nfa_hci_cb.timer);
                if (NFA_HCI_STATE_WAIT_RAPDU == nfa_hci_cb.hci_state) {
                    nfa_hci_cb.hci_state = NFA_HCI_STATE_IDLE;
                    evt_data.rcv_apdu.status = NFA_STATUS_OK;
                    evt_data.rcv_apdu.host_id = p_pipe->dest_host;
                    evt_data.rcv_apdu.p_rsp = p_data;
                    evt_data.rcv_apdu.rsp_len = data_len;
                    nfa_hciu_send_to_all_apps(NFA_HCI_R_APDU_EVT, &evt_data);
                } else {
                    LOG(ERROR) << StringPrintf("hci stack is not waiting for rAPDU, drop it");
                }
                break;
        }
    }
...
**
** Description      send apdu through apdu pipe
**
** Returns          return true on message sent success
**
*******************************************************************************/
bool nfa_hci_api_send_apdu(tNFA_HCI_EVENT_DATA *p_evt_data)
{
    tNFA_STATUS status;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_api_send_apdu");

    if (nfa_hci_cb.hci_state == NFA_HCI_STATE_IDLE) {
        status = nfa_hciu_send_msg (NFA_HCI_APDU_PIPE_ID, NFA_HCI_EVENT_TYPE, NFA_HCI_EVT_C_APDU,
        p_evt_data->send_apdu.capdu_len, p_evt_data->send_apdu.p_capdu);
        if (NFA_STATUS_OK != status) {
            LOG(ERROR) << StringPrintf("nfa_hci_api_send_apdu: send message failed");
            nfa_hci_handle_get_rapdu_timeout();
            goto exit;
        }
        nfa_hci_cb.hci_state = NFA_HCI_STATE_WAIT_RAPDU;
        nfa_sys_start_timer(&nfa_hci_cb.timer, NFA_HCI_RSP_TIMEOUT_EVT,
            p_evt_data->send_apdu.response_timeout);
    } else {
        LOG(WARNING) << StringPrintf("nfa_hci_api_send_apdu: warning:hci state is not expected. state:%d => %s",
            nfa_hci_cb.hci_state, nfa_hciu_get_state_name(nfa_hci_cb.hci_state).c_str());
        nfa_hci_handle_get_rapdu_timeout();
    }
...
/*******************************************************************************
**
** Function         nfa_hci_api_end_apdu
**
** Description      send end of apdu transfer request
**
** Returns          return true on message sent success
**
*******************************************************************************/
bool nfa_hci_api_end_apdu(tNFA_HCI_EVENT_DATA *p_evt_data)
{
    UNUSED(p_evt_data);
    tNFA_STATUS status;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_end_apdu");

    status = nfa_hciu_send_msg (NFA_HCI_APDU_PIPE_ID, NFA_HCI_EVENT_TYPE,
        NFA_HCI_EVT_END_OF_APDU_TRANSACTION, 0, NULL);
    if (NFA_STATUS_OK != status) {
        LOG(ERROR) << StringPrintf("nfa_hci_end_apdu: send message failed");
    }
...
/*******************************************************************************
**
** Function         nfa_hci_handle_get_atr_timeout
**
** Description      dispatch the abnormal status to upper layer
**
*******************************************************************************/
void nfa_hci_handle_get_atr_timeout()
{
    tNFA_HCI_EVT_DATA evt_data;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_handle_get_atr_timeout");

    evt_data.abort_apdu.status = NFA_STATUS_FAILED;
    evt_data.abort_apdu.p_atr = nullptr;
    evt_data.abort_apdu.atr_len = 0;
    nfa_hciu_send_to_all_apps(NFA_HCI_ABORT_APDU_EVT, &evt_data);
}

/*******************************************************************************
**
** Function         nfa_hci_handle_get_rapdu_timeout
**
** Description      dispatch the abnormal status to upper layer
**
*******************************************************************************/
void nfa_hci_handle_get_rapdu_timeout()
{
    tNFA_HCI_EVT_DATA evt_data;
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("nfa_hci_handle_get_rapdu_timeout");

    evt_data.rcv_apdu.status = NFA_STATUS_FAILED;
    evt_data.rcv_apdu.p_rsp = nullptr;
    evt_data.rcv_apdu.rsp_len = 0;
    nfa_hciu_send_to_all_apps(NFA_HCI_R_APDU_EVT, &evt_data);
}
