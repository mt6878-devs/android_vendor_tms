...
**
** Description      Send abort apdu request
**                  result will notify by NFA_HCI_ABORT_APDU_EVT
**
** Returns          NFA_STATUS_OK if message sent success
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_HciAbortApdu(tNFA_HANDLE hci_handle, uint8_t host_id, uint32_t rsp_timeout) {
    tNFA_HCI_API_ABORT_APDU_EVT *p_msg;

    DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("NFA_HciAbortApdu hci_handle:%04X host_id:%02X timeout:%d"
          , hci_handle, host_id, rsp_timeout);
    // check if HCI stack is active
    if (NFA_HCI_STATE_DISABLED == nfa_hci_cb.hci_state) {
        LOG(ERROR) << StringPrintf("%s: HCI stack is not activated", __func__);
        return (NFA_STATUS_FAILED);
    }

    // check if EE is activated
    if (!nfa_hciu_is_active_host(host_id)) {
        LOG(ERROR) << StringPrintf("%s: %d is not activated", __func__, host_id);
        return (NFA_STATUS_FAILED);
    }
    ...
    if (nullptr != p_msg) {
        ...
        p_msg->response_timeout = rsp_timeout;
        nfa_sys_sendmsg(p_msg);
        return (NFA_STATUS_OK);
    }
...
**                  result will notify by NFA_HCI_R_APDU_EVT
**
** Returns          NFA_STATUS_OK if message sent success
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_HciSendApdu(tNFA_HANDLE hci_handle, uint8_t host_id,
    uint8_t *p_capdu, uint16_t capdu_len,
    uint32_t rsp_timeout) {
    tNFA_HCI_API_SEND_APDU_EVT *p_msg;

    DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("NFA_HciSendApdu hci_handle:%04X host_id:%02X capdu_len:%d timeout:%d"
          , hci_handle, host_id, capdu_len, rsp_timeout);
    ...
    if (nullptr != p_msg) {
        ...
        p_msg->hci_handle = hci_handle;
        p_msg->host_id = host_id;
        p_msg->p_capdu = p_capdu;
        p_msg->capdu_len = capdu_len;
        p_msg->response_timeout = rsp_timeout;
        nfa_sys_sendmsg(p_msg);
        return (NFA_STATUS_OK);
    }
...
/*******************************************************************************
**
** Function         NFA_HciEndApdu
**
** Description      Send end of apdu transfer request
**                  No result will receive from NFCC
**
** Returns          NFA_STATUS_OK if message sent success
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFA_STATUS NFA_HciEndApdu(tNFA_HANDLE hci_handle, uint8_t host_id) {
    tNFA_HCI_API_END_APDU_EVT *p_msg;

    DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("NFA_HciEndApdu hci_handle:%04X host_id:%02X"
          , hci_handle, host_id);
    ...
    p_msg = (tNFA_HCI_API_END_APDU_EVT *)GKI_getbuf (sizeof (tNFA_HCI_API_END_APDU_EVT));
    if (nullptr != p_msg) {
        p_msg->hdr.event = NFA_HCI_API_END_APDU_EVT;
        p_msg->hci_handle = hci_handle;
        p_msg->host_id = host_id;
        nfa_sys_sendmsg(p_msg);
        return (NFA_STATUS_OK);
