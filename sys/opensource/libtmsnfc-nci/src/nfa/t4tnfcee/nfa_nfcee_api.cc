...
tNFA_STATUS NFA_T4tNfcEeWrite(uint8_t* p_fileId, uint8_t* p_data,
  ...
  DUMP_PACKET("p_fileId", p_fileId, 2);
  DUMP_PACKET("p_data", p_data, len);
...
/*******************************************************************************
**
** Function         NFA_T4tNfcEeSendApdu
**
** Description      Send raw APDU to the T4T NFC EE.
**
** Returns:         NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
**
*******************************************************************************/
tNFC_STATUS NFA_T4tNfcEeSendApdu(uint8_t* apdu, uint32_t len) {
  tNFA_T4TNFCEE_OPERATION* p_msg;

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s : Enter ", __func__);

  DUMP_PACKET("apdu", apdu, len);

  if ((p_msg = (tNFA_T4TNFCEE_OPERATION*)GKI_getbuf(
           (uint16_t)(sizeof(tNFA_T4TNFCEE_OPERATION)))) != NULL) {
    p_msg->hdr.event = NFA_T4TNFCEE_OP_REQUEST_EVT;
    p_msg->op = NFA_T4TNFCEE_OP_SEND_APDU;
    p_msg->write.len = len;
    p_msg->write.p_data = apdu;
