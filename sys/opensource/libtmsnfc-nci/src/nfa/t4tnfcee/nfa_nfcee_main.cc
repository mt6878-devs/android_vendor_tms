...
void nfa_t4tnfcee_init(void) {
  if (NfcConfig::hasKey(NAME_TMS_T4T_NFCEE_ENABLE)) {
    if (NFA_T4T_NFCEE_ENANLE_BIT_POS & NfcConfig::getUnsigned(NAME_TMS_T4T_NFCEE_ENABLE)) {
...
void nfa_t4tnfcee_info_cback(tNFA_EE_EVT event, tNFA_EE_CBACK_DATA* p_data) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s event: %d, state: %d", __func__, event, nfa_t4tnfcee_cb.t4tnfcee_state);
  switch (event) {
...
static std::string nfa_t4tnfcee_evt_2_str(uint16_t event) {
  switch (event) {
    case NFA_T4TNFCEE_OP_REQUEST_EVT:
