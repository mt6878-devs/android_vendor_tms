...
#ifdef TMS_NFC
    /* Ensure pending power state is set to invalid when setting a new power state */
...
bool nfa_dm_act_send_raw_ptm(tNFA_DM_MSG* p_data) {
  NFC_HDR* p_cmd = (NFC_HDR*)p_data;

  p_cmd->offset = sizeof(tNFA_DM_API_SEND_VSC) - NFC_HDR_SIZE;
  p_cmd->len = p_data->send_vsc.cmd_params_len;
  nfc_ncif_check_cmd_queue(p_cmd);
  ...
  switch (event) {
    ...
    case NFA_DM_RF_DISC_ACTIVATED_EVT:
      ...
      if (nfa_dm_cb.p_activate_ntf) {
        ...
        } else if ((nfa_dm_cb.disc_cb.activated_protocol == NFC_PROTOCOL_UNKNOWN) &&
                 (nfa_dm_cb.disc_cb.activated_rf_interface == NFC_DISCOVERY_TYPE_POLL_B)) {
          /* Notify NFA tag sub-system */
          nfa_rw_proc_disc_evt(NFA_DM_RF_DISC_ACTIVATED_EVT, p_data, true);
#endif
