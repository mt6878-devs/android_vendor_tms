...
#ifdef TMS_NFC
uint8_t tms_rf_listen_mask = NFA_TMS_RF_LISTEN_NOT_SPECIFIED;
#endif
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfa_dm_set_rf_listen_mode_config tech_proto_mask = 0x%08X",
      tech_proto_mask);
#endif
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("tms_rf_listen_mask = %d", tms_rf_listen_mask);
  if (tms_rf_listen_mask != NFA_TMS_RF_LISTEN_NOT_SPECIFIED) {
    if (tms_rf_listen_mask & NFA_TMS_RF_LISTEN_MASK_A) {
      dm_disc_mask |= (NFA_DM_DISC_MASK_LA_T2T |
       NFA_DM_DISC_MASK_LA_ISO_DEP | NFA_DM_DISC_MASK_LA_NFC_DEP);
    } else {
      dm_disc_mask &= ~(NFA_DM_DISC_MASK_LA_T2T |
       NFA_DM_DISC_MASK_LA_ISO_DEP | NFA_DM_DISC_MASK_LA_NFC_DEP);
    }
    if (tms_rf_listen_mask & NFA_TMS_RF_LISTEN_MASK_B) {
      dm_disc_mask |= NFA_DM_DISC_MASK_LB_ISO_DEP;
    } else {
      dm_disc_mask &= ~NFA_DM_DISC_MASK_LB_ISO_DEP;
    }
    if (tms_rf_listen_mask & NFA_TMS_RF_LISTEN_MASK_F) {
      dm_disc_mask |= (NFA_DM_DISC_MASK_LF_T3T | NFA_DM_DISC_MASK_LF_NFC_DEP);
    } else {
      dm_disc_mask &= ~(NFA_DM_DISC_MASK_LF_T3T | NFA_DM_DISC_MASK_LF_NFC_DEP);
    }
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s - apply tms rf listen mask, dm_disc_mask : 0x%x", __func__, dm_disc_mask);
  }
#endif
