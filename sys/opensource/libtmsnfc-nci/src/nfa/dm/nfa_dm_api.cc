...
extern uint8_t tms_rf_listen_mask;
#endif
...
#ifdef TMS_NFC
/*******************************************************************************
** Function:        NFA_SetTmsRfListenMask
**
** Description:     This function set rf listen mask
**
** Returns:        NFA_STATUS_OK if successfully initiated
**                  NFA_STATUS_FAILED otherwise
*******************************************************************************/
tNFA_STATUS NFA_SetTmsRfListenMask(uint8_t listenMask) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("NFA_SetTmsRfListenMask listenMask=%d", listenMask);
  if (listenMask == NFA_TMS_RF_LISTEN_NOT_SPECIFIED) {
    tms_rf_listen_mask = NFA_TMS_RF_LISTEN_NOT_SPECIFIED;
  } else {
    uint8_t i = 0;
    if (listenMask & NFA_TMS_RF_LISTEN_MASK_A) {
      i |= NFA_TMS_RF_LISTEN_MASK_A;
    }
    if (listenMask & NFA_TMS_RF_LISTEN_MASK_B) {
      i |= NFA_TMS_RF_LISTEN_MASK_B;
    }
    if (listenMask & NFA_TMS_RF_LISTEN_MASK_F) {
      i |= NFA_TMS_RF_LISTEN_MASK_F;
    }
    tms_rf_listen_mask = i;
  }
  return NFA_STATUS_OK;
}

/*******************************************************************************
** Function:        NFA_GetTmsRfListenMask
**
** Description:     This function get the current rf listen mask
**
** Returns:         RF listen mask
*******************************************************************************/
uint8_t NFA_GetTmsRfListenMask() {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("NFA_GetTmsRfListenMask listenMask=%d", tms_rf_listen_mask);
  return tms_rf_listen_mask;
}

tNFA_STATUS NFA_WaitVsResponse(tNFA_VSC_CBACK* p_cback) {
    DLOG_IF(INFO, nfc_debug_enabled) << __func__;
    NFC_MarkWaitVsResponse((void*)p_cback);
    return NFA_STATUS_OK;
}

tNFA_STATUS NFA_SetPassthroughMode(uint8_t mode, tNFA_VSC_CBACK* p_cback) {
  DLOG_IF(INFO, nfc_debug_enabled) << __func__;
  NFC_SetPassthroughMode(mode, (void*)p_cback);
  return NFA_STATUS_OK;
}

tNFA_STATUS NFA_SendRawPtmCommand(uint16_t cmd_params_len,uint8_t* p_cmd_params) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s : len = %d", __func__, cmd_params_len);
  if (cmd_params_len == 0x00 || p_cmd_params == nullptr) {
    return NFA_STATUS_INVALID_PARAM;
  }
  uint16_t size = sizeof(tNFA_DM_API_SEND_VSC) + cmd_params_len;
  tNFA_DM_API_SEND_VSC* p_msg = (tNFA_DM_API_SEND_VSC*)GKI_getbuf(size);
  ...
  if (p_msg != nullptr) {
    p_msg->hdr.event = NFA_DM_API_SEND_RAW_PTM_EVT;
    p_msg->cmd_params_len = cmd_params_len;
    p_msg->p_cmd_params = (uint8_t*)(p_msg + 1);
    memcpy(p_msg->p_cmd_params, p_cmd_params, cmd_params_len);
