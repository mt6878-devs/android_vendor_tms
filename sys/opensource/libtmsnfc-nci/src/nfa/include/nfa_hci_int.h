...
#ifdef TMS_NFC
/* HCI is waiting for ATR from apdu pipe */
#define NFA_HCI_STATE_WAIT_ATR 0x0A
/* HCI is waiting for RAPDU from apdu pipe */
#define NFA_HCI_STATE_WAIT_RAPDU 0x0B
#endif
...
#ifdef TMS_NFC
      NFA_HCI_API_ABORT_APDU_EVT, /* send abort apdu request */
      NFA_HCI_API_SEND_APDU_EVT, /* send apdu through APDU pipe */
      NFA_HCI_API_END_APDU_EVT, /* end of APDU transfer */
#endif
...
#else
#define NFA_HCI_LAST_API_EVENT NFA_HCI_API_END_APDU_EVT
#endif
...
#ifdef TMS_NFC
/* data type for NFA_HCI_API_ABORT_APDU_EVT */
typedef struct {
    NFC_HDR hdr;
    tNFA_HANDLE hci_handle;
    uint8_t host_id;
...
/* data type for NFA_HCI_SEND_APDU_EVT */
typedef struct {
    NFC_HDR hdr;
    tNFA_HANDLE hci_handle;
    uint8_t host_id;
...
/* data type for NFA_HCI_API_END_APDU_EVT */
typedef struct {
    NFC_HDR hdr;
    tNFA_HANDLE hci_handle;
    uint8_t host_id;
} tNFA_HCI_API_END_APDU_EVT;
#endif
...
#ifdef TMS_NFC
    tNFA_HCI_API_ABORT_APDU_EVT abort_apdu; /* send abort apdu request */
    tNFA_HCI_API_SEND_APDU_EVT send_apdu; /* send apdu through APDU pipe */
    tNFA_HCI_API_END_APDU_EVT end_apdu; /* end of APDU transfer */
#endif
} tNFA_HCI_EVENT_DATA;
...
#ifdef TMS_NFC
tNFA_STATUS nfa_hciu_send_raw_data(uint8_t *p_buf, uint8_t data_len, tNFA_VSC_CBACK * p_cback);
