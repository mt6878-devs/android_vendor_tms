...
#ifdef TMS_NFC
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s , RW_T4T_INTF_ERROR_EVT status = %d", __func__, conn_evt_data.status);
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s operation=%d", __func__, p_data->op_req.op);
#endif
...
#ifdef TMS_NFC
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s appl_dta_mode_flag:%d",
