...
//TMS_NFC for route setting
#ifdef TMS_NFC
  } else if ((sizeof (NFA_REMOVE_ALL_AID) == p_data->rm_aid.aid_len)
  ...
  if (p_cb) {
    ...
    if (p_ee->nfcee_id == T4TNFCEE_TARGET_HANDLE) {
      if (NfcConfig::hasKey(NAME_TMS_T4T_NFCEE_ENABLE) &&
          NfcConfig::getUnsigned(NAME_TMS_T4T_NFCEE_ENABLE) == 0x01) {
...
void nfa_ee_pwr_and_link_ctrl_rsp(tNFA_EE_MSG* p_data) {
  ...
  if (p_data != nullptr) {
#ifdef TMS_NFC
    if (p_data->pwr_and_link_ctrl.p_data != nullptr) {
      evt_data.status = p_data->pwr_and_link_ctrl.p_data->status;
    } else {
      evt_data.status = NFA_STATUS_FAILED;
      DLOG_IF(INFO, nfc_debug_enabled) << __func__ << StringPrintf("set PL result is null");
    }
#else
    evt_data.status = NFA_STATUS_OK;
#endif
