    ...
    case RW_MFC_STATE_DETECT_MAD:
      ...
      if (p_mfc->current_block == 1 || p_mfc->current_block == 64 ||
        ...
        for (int k = 0; k < 16; k++) {
          DLOG_IF(INFO, nfc_debug_enabled)
              << __func__ << " sector= " << k
              << " is_ndef = " << p_mfc->mifare_ndefsector[k];
        }
...
static void rw_mfc_process_error() {
...
#ifdef TMS_NFC
  bool is_nack = (p_mfc->substate == RW_MFC_SUBSTATE_WAIT_ACK);
#endif
  ...
  if (
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << __func__ << "Is Nack recived, no need to retry";
#endif
  } else {
    DLOG_IF(INFO, nfc_debug_enabled)
...
static tNFC_STATUS rw_MfcCheckMad() {
  ...
  if (success == NFC_STATUS_OK) {
    p_mfc->state = RW_MFC_STATE_DETECT_MAD;
    DLOG_IF(INFO, nfc_debug_enabled)
        << __func__ << " rw_MfcCheckMad state=" << p_mfc->state;
  } else {
    p_mfc->substate = RW_MFC_SUBSTATE_NONE;
    DLOG_IF(INFO, nfc_debug_enabled)
