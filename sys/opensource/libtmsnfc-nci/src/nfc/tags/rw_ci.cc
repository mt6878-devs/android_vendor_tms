...
/******************************************************************************
 *
 *  This file contains the implementation for China ID Card in Reader/Writer
 *  mode.
 *
 ******************************************************************************/
#include <string.h>
  ...
  if (rw_cb.p_cback) {
    ...
    switch (p_ci->state) {
      ...
      case RW_CI_STATE_PRESENCE_CHECK:
        ...
        if (status == NFC_STATUS_TIMEOUT) {
          rw_data.status = NFC_STATUS_TIMEOUT;
        } else {
...
tNFC_STATUS RW_CiPresenceCheck(void) {
  ...
  /* If command is pending, assume tag is still present */
    ...
    if ((p_data = (NFC_HDR*)GKI_getpoolbuf(NFC_RW_POOL_ID)) != nullptr) {
