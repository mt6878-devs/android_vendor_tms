...
#ifdef TMS_NFC
    case NCI_HAL_INIT_FAILED_EVT:
      return "NCI_HAL_INIT_FAILED_EVT";
#endif
    default:
...
static void nfc_main_hal_cback(uint8_t event, tHAL_NFC_STATUS status) {
  ...
  switch (event) {
    ...
    case NCI_HAL_INIT_FAILED_EVT:
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("nfc_main_hal_cback abort");
      abort();
      break;
#endif
...
#ifdef TMS_NFC
void NFC_MarkWaitVsResponse(void* p_cback) {
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("NFC_MarkWaitVsResponse() Enter");
  /* save the callback for RAW VS */
  nfc_cb.p_vsc_cback = p_cback;
  nfc_cb.rawVsCbflag = true;
}

void NFC_SetPassthroughMode(uint8_t mode, void* p_cback) {
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf("NFC_SetPassthroughMode() Enter, mode = %d", mode);
  nfc_cb.passthroughMode = mode;
  nfc_cb.p_ptm_cback = p_cback;
}
#endif