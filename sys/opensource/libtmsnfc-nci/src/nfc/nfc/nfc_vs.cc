...
tNFC_STATUS NFC_RegVSCback(bool is_register, tNFC_VS_CBACK* p_cback) {
...
#else
    for (i = 0; i < NFC_NUM_VS_CBACKS; i++) {
      if (status != NFC_STATUS_OK && nfc_cb.p_vs_cb[i] == nullptr) {
        /* found an empty spot to hold the callback function */
        nfc_cb.p_vs_cb[i] = p_cback;
        status = NFC_STATUS_OK;
      } else if (status != NFC_STATUS_OK && nfc_cb.p_vs_cb[i] == p_cback) {
        /* already registered */
        status = NFC_STATUS_OK;
      } else if (status == NFC_STATUS_OK && nfc_cb.p_vs_cb[i] == p_cback) {
        /* clear repeated callbacks */
  ...
  if (status != NFC_STATUS_OK) {
    LOG(ERROR) << StringPrintf("%s: failed, is_register = %d NFC_NUM_VS_CBACKS=%d", __func__, is_register, NFC_NUM_VS_CBACKS);
  }
#endif
  return status;
}
