...
#ifdef TMS_NFC
static tNFC_NFCC_MANUF_SPEC_INFO nfcc_infos;
#endif
...
#ifdef TMS_NFC
        nfc_cb.rawVsGid = nfc_cb.last_hdr[0] & NCI_GID_MASK;
        nfc_cb.rawVsOid = nfc_cb.last_hdr[1] & NCI_OID_MASK;
#endif
      }
...
#ifdef TMS_NFC
      if (nfc_cb.last_hdr[0] & NCI_PBF_MASK) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("nfc_ncif_check_cmd_queue : is segment, do not wait rsp");
        nfc_cb.nci_cmd_window++;
      } else {
#endif
...
#ifdef TMS_NFC
tNFC_NFCC_MANUF_SPEC_INFO NFC_GetManufSpecInfo() { return nfcc_infos; }
#endif
...
#ifdef TMS_NFC
bool nci_proc_passthrough_mode(NFC_HDR* p_msg) {
    uint8_t op_code;

  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s enter, mode = %d", __func__, nfc_cb.passthroughMode);

  if (nfc_cb.passthroughMode == 0) {
    return false;
  }
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s head = %d", __func__, head);

  if (nfc_cb.passthroughMode == 1 && ((head & 0xE0) == 0) && ((head & 0x0F) == 1)) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s is hci data", __func__);
    tNFC_VS_CBACK* p_cback = (tNFC_VS_CBACK*)nfc_cb.p_ptm_cback;
    ...
    * callback function */
    ...
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s not match", __func__);
    return false;
  }
}
#endif
...
bool nfc_ncif_process_event(NFC_HDR* p_msg) {
  ...
  if (nfc_cb.passthroughMode != 0) {
    // PASSTHROUGH MODE
    if (nci_proc_passthrough_mode(p_msg)) {
      return free;
    }
  }
#endif
...
#ifdef TMS_NFC
      nfc_cb.rawVsGid == gid && nfc_cb.rawVsOid == oid &&
#endif
...
#ifdef TMS_NFC
      p++; // p points Manufacturer ID
      STREAM_TO_UINT8(nfcc_infos.len, p);
      LOG(INFO) << StringPrintf("%s: nfcc_manuf_spec_info_len: %d", __func__, nfcc_infos.len);
      if (nfcc_infos.len > MAX_NFCC_MANUF_SPEC_INFO_LEN) {
        nfcc_infos.len = MAX_NFCC_MANUF_SPEC_INFO_LEN;
      }
      STREAM_TO_ARRAY(nfcc_infos.infos, p, nfcc_infos.len);
#endif
    } else {
