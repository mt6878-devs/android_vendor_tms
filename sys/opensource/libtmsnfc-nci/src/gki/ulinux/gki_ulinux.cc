...
void* gki_task_entry(void* params) {
  ...
  // reset thread_id after pthread_join in GKI_shutdown
#else
...
#else
      gki_cb.os.thread_id[task_id - 1] = 0;
#endif
#endif
      DLOG_IF(INFO, nfc_debug_enabled)
...
#ifdef TMS_NFC
  pthread_mutex_destroy(&gki_cb.os.gki_timer_mutex);
  pthread_cond_destroy(&gki_cb.os.gki_timer_cond);
#endif
#if (FALSE == GKI_PTHREAD_JOINABLE)
  i = 0;
#endif
