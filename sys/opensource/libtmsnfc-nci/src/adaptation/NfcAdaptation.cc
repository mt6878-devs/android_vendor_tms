...
sp<ITmsNfc> NfcAdaptation::mHalTmsNfc;
#ifdef TMS_NFC_AIDL
using ITmsNfcAidl = ::aidl::vendor::tms::tmsnfc_aidl::ITmsNfc;
std::shared_ptr<ITmsNfcAidl> mAidlHalTmsNfc;
...
#ifdef TMS_NFC
uint8_t appl_dta_t5t_mode_flag = 0x00;
#endif
...
void NfcAdaptation::Initialize() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NCI MW Version: %s", func, mw_version);
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: NCI MW Build Time: %s", func, mw_build_time);
#endif
...
#if defined (TMS_NFC) && (GKI_PTHREAD_JOINABLE != FALSE)
  // GKI_exit_task(GKI_get_taskid());    // exit MMI_task after pthread_join
#else
...
#ifdef TMS_NFC
          android::base::GetProperty(VERBOSE_VENDOR_LOG_PROPERTY, "true")
#else
 ...
 *******************************************************************************/
std::string NfcAdaptation::HalGetProperty(std::string key) {
  std::string value;
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: enter key %s", __func__, key.c_str());
#ifdef TMS_NFC_AIDL
  if (mAidlHalTmsNfc != nullptr) {
    mAidlHalTmsNfc->getVendorParam(key, &propVal);
  ...
  if (mHalTmsNfc != NULL) {
    ...
    mHalTmsNfc->getVendorParam(key, HalGetProperty_cb);
    value = propVal;    /* Copy the string received from HAL */
    propVal.assign(""); /* Clear the global string variable  */
  } else {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: mHalTmsNfc is NULL", __func__);
  }
...
#ifdef TMS_NFC_AIDL
  if (mAidlHalTmsNfc != nullptr) {
     mAidlHalTmsNfc->setTmsTransitConfig(strval, &status);
  } else
#endif
  if (mHalTmsNfc != NULL) {
    status = mHalTmsNfc->setTmsTransitConfig(strval);
  } else {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: mHalTmsNfc is NULL", __func__);
  }
...
bool NfcAdaptation::HalCheckIfConfigIsModified() {
    bool result = false;
#ifdef TMS_NFC_AIDL
    if (mAidlHalTmsNfc != nullptr) {
        mAidlHalTmsNfc->isConfigModified(FILE_TYPE_TMS, &result);
        if (!result) {
            mAidlHalTmsNfc->isConfigModified(FILE_TYPE_RF, &result);
        }
    } else
#endif
    if (mHalTmsNfc != NULL) {
        result = mHalTmsNfc->isConfigModified(FILE_TYPE_TMS);
        if (!result) {
            result = mHalTmsNfc->isConfigModified(FILE_TYPE_RF);
        }
    } else {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: mHalTmsNfc is NULL", __func__);
    ...
    if (result) {
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s: config has modifdied", __func__);
    }
    return result;
}
#endif
...
/*******************************************************************************
**
** Function:    NfcAdaptation::InitializeTmsHalContext
**
** Description: Get the TmsNfc HIDL or AIDL interface Service
**
** Returns:     None.
**
*******************************************************************************/
void NfcAdaptation::InitializeTmsHalContext() {
  const char* func = "NfcAdaptation::InitializeTmsHalContext";
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s", func);
  if (!use_aidl) {
    LOG(INFO) << StringPrintf("%s: ITmsNfc::tryGetService()", func);
    mHalTmsNfc = ITmsNfc::tryGetService();
    if (mHalTmsNfc != nullptr) {
      LOG(INFO) << StringPrintf("%s: ITmsNfc::getService() returned %p (%s)", func,
        mHal.get(), (mHal->isRemote() ? "remote" : "local"));
    }
  } else {
#ifdef TMS_NFC_AIDL
    ...
    if (binder != nullptr) {
      mAidlHalTmsNfc = ITmsNfcAidl::fromBinder(binder);
      if (mAidlHalTmsNfc != nullptr) {
        LOG(INFO) << StringPrintf("%s: ITmsNfcAidl::fromBinder returned", func);
      } else {
        LOG(ERROR) << StringPrintf("%s: Failed to retrieve the TmsNfc AIDL", func);
      }
    } else {
      LOG(ERROR) << StringPrintf("%s: Faild to get tms nfc aidl service binder ", func);
    }
#endif
  }
}
