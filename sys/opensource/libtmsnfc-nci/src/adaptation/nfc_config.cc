...
#ifdef TMS_NFC
#define PATH_TRANSIT_CONF "/data/nfc/libnfc-tmsTransit.conf"
ThreadMutex NfcConfig::sLock;
#endif
namespace {
std::string searchConfigPath(std::string file_name) {
  const std::vector<std::string> search_path = {
      "/data/nfc/", "/product/etc/", "/odm/etc/", "/vendor/etc/", "/system_ext/etc/", "/etc/",
  };
...
void NfcConfig::loadConfig() {
  ...
  std::string tms_config;
  tms_config = theInstance.HalGetProperty("libnfc-tms.conf");
  config_.cur_file_name_ = "tmsConfig";
  config_.parseFromString(tms_config);
